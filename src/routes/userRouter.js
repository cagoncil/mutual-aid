...
/* This is a route handler for the route /profile. It is a GET request. It is using the
userController.authenticateUser middleware function to check if the user is authenticated. If the
user is authenticated, it will send the user object to the client. */
...
router.get('/account', userController.authenticateUser, (req, res) => {
  return res
    .status(200)
    .sendFile(path.resolve(__dirname, '../../client/account.html'));
});
...
router.get('/clinics', userController.authenticateUser, (req, res) => {
  return res
    .status(200)
    .sendFile(path.resolve(__dirname, '../../client/clinics.html'));
});
...
router.get('/statelaws', userController.authenticateUser, (req, res) => {
  return res
    .status(200)
    .sendFile(path.resolve(__dirname, '../../client/statelaws.html'));
});
...
router.get('/help', userController.authenticateUser, (req, res) => {
  return res
    .status(200)
    .sendFile(path.resolve(__dirname, '../../client/help.html'));
});
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (updatedItem === 'email') {
        ...
        if (authenticated && matchingInputs)
...
router.get('/error', (req, res) => {
  return res
    .status(200)
    .sendFile(path.resolve(__dirname, '../../client/error.html'));
});
...
router.use((err, req, res, next) => {
  ...
  if (err.errors !== undefined) {
    ...
    errMsg =
      'Registration failed: a user is already registered with that email.';
